...
| **1.66.0** | 2022/03/15 | 吳志豪 | (1)調整 五.各付費方式延伸欄位說明表 中MyCard 儲值卡CardType資訊 |
...
| 馬來：<br />Touch 'n Go(CodaShop) | (二) 確認請求電文<br/>前端選擇品項的檢驗，CENTER_TRANS_NO<br/>不會帶值 | 儲值中心<br>↓<br>廠商 | PRODUCT_ID=1&USER_ID=xxxxxxxx<br><br />PRODUCT_ID：雙方定義的品項ID，varchar(20)<br>USER_ID：欲儲值的帳號 |
| 馬來：<br />Touch 'n Go(CodaShop) | (二) 確認請求電文 | 廠商<br/>↓<br/>儲值中心 | [用戶存在且品項有資格購買]<br />IDENTITY_1=yyy&IDENTITY_2=zzz&USER_NAME=xxx<br/><br />IDENTITY_1：廠商用戶識別值1，varchar(100)<br/>IDENTITY_2：廠商用戶識別值2，varchar(100)<br/>USER_NAME：玩家暱稱 (顯示在 CodaShop 介面上給用戶確認身分的，避免除錯帳號，看遊戲有什麼可以識別，例如暱稱)，varchar(100)<br /><br />[其他錯誤不給購買]<br />CODA_ERROR_CODE={錯誤代碼}&CODA_ERROR_MSG={錯誤訊息}<br />CODA_ERROR_CODE：CodaShop 會依據此代碼顯示錯誤訊息給用戶，varchar(20)<br/>CODA_ERROR_MSG：錯誤訊息，串接雙方內部使用，不會給用戶看到<br /><br />CODA_ERROR_CODE定義<br />-100 用戶不存在<br />-101 品項不存在<br />-102 品項不符合購買資格<br />-103 其他錯誤<br />-104 用戶未滿足條件<br />-105 須保持遊戲登入<br />-106 帳號未創建該伺服器角色<br />-107 已購買過此品項，不可重複買<br />-200 用戶不可在此[國家/地區]消費 |
| 馬來：<br />Touch 'n Go(CodaShop) | (三) 交易結果電文<br/>(四) 交易回報確認電文 | 儲值中心<br/>↓<br/>廠商 | AMOUNT=500000.00000<br/>&CURRENCY_ID=IDR&CURRENCY_RATE= 0.00319&NT_AMOUNT=1595.00000&IS_CODASHOP=True<br/><br/>AMOUNT：品項金額<br/>CURRENCY_ID：幣別ID<br/>CURRENCY_RATE：匯率<br/>NT_AMOUNT：換算台幣金額<br/>IS_CODASHOP：是否為 CodaShop模式<br/><br/>注意：<br/>當RESULT_CODE=000000時，才會帶值<br/>若訂單尚未確認已完成，會回傳RESULT_CODE=000005(交易尚未確認完成，請稍等片刻再確認您的帳戶和遊戲幣，如有問題請洽客服。) 此為『前端成功回覆代碼，但是不可作為請款依據，須依造(四) 交易回報確認電文回覆成功才給點(000000 or 000001)』 |
